...
export default class Bubble extends React.Component {
    ...
    renderSendFailed() {
        if (this.props.currentMessage.status == 'failed'&&this.props.renderSendFailed && this.props.position=='right') {
            return this.props.renderSendFailed(this.props);
        }
        return null;
    }
    renderCustomView() {
    ...
    render() {
        return (
            <View style={[{flexDirection:'row'}]}>
                {this.props.position=='right'?<View style={{flex:1}}/>:null}
                {this.renderSendFailed()}
                ...
                {this.props.position=='left'?<Text style={{display:'none',backgroundColor:'green'}}>已读</Text>:null}
                {this.props.position=='left'?<View style={{flex:1}}/>:null}
            </View>
        );
    }
}
